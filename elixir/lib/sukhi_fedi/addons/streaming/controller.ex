...
defmodule SukhiFedi.Addons.Streaming.Controller do
  import Plug.Conn
  alias SukhiFedi.Addons.Streaming.Registry
  alias SukhiFedi.Auth

  def stream(conn) do
    case conn.path_params["urn"] do
  ...
  defp stream_home(conn) do
    case authenticate(conn) do
      {:ok, account} -> stream_sse(conn, :home, account.id)
      {:error, :unauthorized} -> send_resp(conn, 401, "Unauthorized")
    end
  end

  defp stream_local(conn), do: stream_sse(conn, :local, nil)
  defp stream_public(conn), do: stream_sse(conn, :public, nil)
