...
defmodule SukhiFedi.Moderation do
  import Ecto.Query
  ...
  def muted?(account_id, target_id) do
    Repo.exists?(
      from m in Mute,
      where: m.account_id == ^account_id and m.target_id == ^target_id and
  ...
  def list_reports(status \\ "open") do
    Repo.all(from r in Report, where: r.status == ^status, order_by: [desc: r.inserted_at], preload: [:account, :target, :note])
  end

  def resolve_report(report_id, resolved_by_id) do
    Repo.get!(Report, report_id)
    |> Ecto.Changeset.change(%{status: "resolved", resolved_at: DateTime.utc_now(), resolved_by_id: resolved_by_id})
    |> Repo.update()
  end
  ...
  def suspend_account(account_id, suspended_by_id, reason) do
    ...
    |> Ecto.Changeset.change(%{suspended_at: DateTime.utc_now(), suspended_by_id: suspended_by_id, suspension_reason: reason})
    |> Repo.update()
  end

  def unsuspend_account(account_id) do
    Repo.get!(Account, account_id)
    |> Ecto.Changeset.change(%{suspended_at: nil, suspended_by_id: nil, suspension_reason: nil})
    |> Repo.update()
  end
end
