...
// instances.
//
// Running: NATS_URL=nats://localhost:4222 deno task fedify-service
...
// NOTE: handleBuildIntegrityProof intentionally omitted — pre-existing TS
// error in handlers/build/integrity_proof.ts:71 (Uint8Array / BufferSource
// incompatibility) blocks type-checking. Callers still reach it via the
// legacy `ap.build.integrity_proof` subscribe in main.ts. Re-import here
// once the type bug is fixed in stage 5 (God-module decomposition).
import { handleSignDelivery } from "../handlers/sign_delivery.ts";
import { handleVerify } from "../handlers/verify.ts";

// deno-lint-ignore no-explicit-any
type AnyMsg = any;
type TranslateHandler = (payload: unknown) => Promise<unknown>;

const dec = new TextDecoder();
const enc = new TextEncoder();

// Dispatch table for `translate.v1`. Keys are ActivityPub-ish domain
// object types; values are the handlers that know how to build JSON-LD
// for that type.
const TRANSLATORS: Record<string, TranslateHandler> = {
  ...
  // integrity_proof: see import comment — added back in stage 5.
};
...
if (import.meta.main) {
  const url = Deno.env.get("NATS_URL") ?? "nats://localhost:4222";
